...
const create = async (payload) => {
  const { nama, jenisKelamin, pendidikan, usia, surveyResult } = await validate(
    surveySchema,
    payload
  );
...
const getById = async (payload) => {
  ...
  const interpretation = (gap, dimension) => {
    ...
    switch (dimension) {
      ...
      default:
        message =
          "Dimensi tidak dikenal: Pastikan dimensi yang diberikan benar.";
    }
  ...
  return {
    ...
    result: data.result.map((item) => {
      const averageExp = average(
        item.expectation.map((answer) => answer.answer)
      );
      const averagePer = average(
        item.perception.map((answer) => answer.answer)
      );
...
const getAllAverage = async () => {
  ...
  const calculateAverage = (answers) => {
    const total = answers.reduce((sum, answerObj) => sum + answerObj.answer, 0);
  ...
  const rangeUsia = {
    less_than_twenty: 0,
    twenty_until_twentyThree: 0,
    over_twentyThree: 0,
  };

  data.map((hasil) => {
    if (hasil.respondent.usia < 20) {
      rangeUsia.less_than_twenty++;
    } else if (hasil.respondent.usia >= 20 && hasil.respondent.usia <= 23) {
      rangeUsia["twenty_until_twentyThree"]++;
    } else {
      rangeUsia.over_twentyThree++;
    }
    // const avverageDetail =
    hasil.result.map((result) => {
      const averageExpectation = calculateAverage(result.expectation);
      const averagePerception = calculateAverage(result.perception);
      const gap = (averagePerception - averageExpectation).toFixed(2);
      dataAverage.push([averageExpectation, averagePerception, gap]);
      return {
    ...
    return {
      ...
      respondent: hasil.respondent,
    };
  });

  const totals = dataAverage.reduce(
    (acc, [expectation, perception, gap]) => {
      acc.totalExpectation += expectation;
      acc.totalPerception += perception;
      acc.totalGap += parseFloat(gap);
      acc.count += 1;
      return acc;
    },
    { totalExpectation: 0, totalPerception: 0, totalGap: 0, count: 0 }
  );

  return {
    averageExpectation: totals.totalExpectation / totals.count,
    averagePerception: totals.totalPerception / totals.count,
    averageGap: totals.totalGap / totals.count,
    averageTotal:
      totals.totalExpectation / totals.count +
      totals.totalPerception / totals.count +
      totals.totalGap / totals.count,
    totalResponden: data.length,
    rangeUsia,
  };
};
